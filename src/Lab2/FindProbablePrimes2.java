...
public class FindProbablePrimes2 implements Runnable, Function<BigInteger, BigInteger> {
    ...
    public void FindProbablePrimes(int id){
        this.id = id;
    }

    public static Map<BigInteger, BigInteger> sequential(List<BigInteger> arguments) {
        Map<BigInteger, BigInteger> map = new HashMap<>();
        for (BigInteger item : arguments)
            map.put(item, item.nextProbablePrime());

        return map;
    }
    public static Map<BigInteger, BigInteger> parallel(
            List<BigInteger> arguments) {
    ...
    public static Map<BigInteger, BigInteger> parallel2(
            List<BigInteger> arguments) {
